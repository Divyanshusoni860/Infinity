...
function evaluateLender(lender, input) {
  ...
  if (eligible) {
    const base =
      lender.calcBasis === "emi"
        ? input.currentEMI * input.tenorServed * multiplier
        : input.valuation * multiplier;
    loanAmount = lender.loanCapping > 0 ? Math.min(base, lender.loanCapping) : base;
    topup = loanAmount - input.closureAmount;

    // Some lenders don't cap the total loan, but do cap the top-up
    // (cash-out) portion itself — e.g. "closure amount + max top-up of ₹XXL".
    ...
    // instead of (or in addition to) an absolute rupee cap.
